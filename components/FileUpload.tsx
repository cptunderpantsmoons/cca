  ...
  return (
    <div className="w-full">
        ...
        <div
          ...
          onDrop={handleDrop}
          className={`
            flex justify-center items-center w-full h-48 px-4 transition bg-gray-800/50 border-2 
            ${isDragging ? 'border-sky-500' : 'border-gray-600'} 
            border-dashed rounded-lg cursor-pointer hover:border-gray-500
          `}
        >
          {selectedFile ? (
             <div className="text-center">
                ...
                <p className="mt-2 text-sm text-gray-300 font-semibold">{selectedFile.name}</p>
                <p className="text-xs text-gray-500">{(selectedFile.size / 1024).toFixed(2)} KB</p>
                <button 
                    onClick={onFileRemove}
                    className="mt-3 text-xs text-red-400 hover:text-red-300 font-bold py-1 px-2 rounded-md bg-red-900/50"
                >
                    Remove File
                </button>
             </div>
